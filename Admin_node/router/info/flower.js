...
flower.post('/add', async (ctx) => {
    ...
    let sql = `INSERT INTO flower (name,florescence,department,allegory,color,fid) VALUES ('${flower_info.name}','${flower_info.florescence}','${flower_info.department}','${flower_info.allegory}','${flower_info.color}','${flower_info.fid}')`
    let result = await new Promise((resolve) => {
        db.query(sql, (err, data) => {
            if (err) throw err;
            resolve(data)
...
flower.post('/addPhoto', async (ctx) => {
    let flower_info = ctx.request.body;
    let sql = `INSERT INTO picture (fid,store,name) VALUES (${flower_info.fid},'${flower_info.store}','${flower_info.name}')`
    let result = await new Promise((resolve) => {
        db.query(sql, (err, data) => {
            if (err) throw err;
            resolve(data)
